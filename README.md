...
- **GitHub Pages:** See [GitHub Pages Deployment](#github-pages-deployment) below
...
- Your repository is set up on GitHub
- You have push access to the repository

#### Step 1: Build for GitHub Pages

Build the site with the correct base path for your repository:
...
This script will:
- Automatically detect your repository name from git (or use "CSE-140-Website" as default)
- Set the correct base path for GitHub Pages
- Fetch Canvas data (if configured)
- Build the site with proper asset paths
- Create a `404.html` file for client-side routing support

**Manual Repository Name:**

If the automatic detection doesn't work, you can specify your repository name:

```bash
bash scripts/build-github-pages.sh YOUR_REPO_NAME
```

For example, if your repository is `username/CSE-140-Website`, use:
```bash
bash scripts/build-github-pages.sh CSE-140-Website
```
...
**Option A: Manual Deployment (gh-pages branch)**
...
3. GitHub Actions will automatically build and deploy on every push to the main branch

#### Troubleshooting

**White Screen on GitHub Pages:**

If you see a white screen after deployment, check:

1. **Base Path:** Ensure the base path matches your repository name
   - If your repo is `username/CSE-140-Website`, the base path should be `/CSE-140-Website/`
   - If your repo is `username.github.io`, the base path should be `/`

2. **Rebuild:** Rebuild with the correct repository name:
   ```bash
   npm run build:github-pages YOUR_REPO_NAME
   ```

3. **404.html:** Ensure `404.html` exists in the `dist/` folder (the build script creates this automatically)

4. **Asset Paths:** Check the browser console for 404 errors on assets. If assets fail to load, the base path is likely incorrect.

**Routes Not Working:**

- Ensure `404.html` exists in your `dist/` folder
- The build script automatically creates this file
- GitHub Pages will serve `404.html` for unknown routes, which redirects to your React app

**Environment Variables:**

If you're using environment variables (like Canvas API tokens), set them in:
- GitHub Actions: Repository Settings → Secrets and variables → Actions
- Or use a `.env` file that's committed (only for non-sensitive variables)
